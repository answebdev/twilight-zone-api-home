...
I started by giving each of the anchor link buttons in `Home.js` a class of `hashed`, in order to be able to select them in the function. Clicking on each of these buttons will navigate the user to the appropriate section of the page, each containing either the link URL of `#docs` or `#resources`, depending on which button was clicked.
...
Back in the `handleHashUrl` component that I created, I began by first converting everything from the original jQuery to JavaScript. I then selected these anchor link buttons with the class of `hashed` and used a `forEach` to loop through them, since I had more than one that I was targeting. A `setTimeout` is needed to give it enough time (in this case, 5 milliseconds) to actually do the anchor scroll before running the `removeHash` function, which is what actually removes the hash from the URL.
